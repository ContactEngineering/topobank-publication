...
def test_sharing_status_filter(api_client, example_authors, handle_usage_statistics):
    ...
    Publication.publish(
        surface_published_egress,
        "cc0-1.0",
        surface_published_egress.creator,
        example_authors,
    )
    ...
    Publication.publish(
        surface_published_ingress,
        "cc0-1.0",
        surface_published_ingress.creator,
        example_authors,
    )
    ...
    result = api_client.get(
        reverse("manager:surface-api-list") + "?sharing_status=all"
    ).data
    assert len(result) == 6  # All

    result = api_client.get(
        reverse("manager:surface-api-list") + "?sharing_status=own"
    ).data
