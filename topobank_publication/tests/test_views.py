...
@pytest.mark.django_db
def test_redirection_invalid_publication_link(
    client, orcid_socialapp, handle_usage_statistics
):
    response = client.get(
        reverse("publication:go", kwargs=dict(short_url="THISISNONSENSE"))
    )
